...
class Logger {
public:
  explicit Logger(::flowmill::ingest::Writer &writer):
    writer_(writer)
  {}
  ...
  template <typename Format, typename... Args>
  inline void critical(Format &&format, Args &&... args) {
    LOG::critical(
      log_message<spdlog::level::critical>(
       std::forward<Format>(format), std::forward<Args>(args)...
      )
    );
  }
...
private:
  ...
  inline std::string log_message(Format &&format, Args &&... args) {
    ...
    static_assert(
      static_cast<std::underlying_type_t<spdlog::level::level_enum>>(Level)
