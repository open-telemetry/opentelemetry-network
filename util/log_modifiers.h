...
constexpr inline bool is_logging_overhead = (
  (std::is_same_v<std::string, std::decay_t<Args>> && std::is_rvalue_reference_v<Args>)
    || ... || false
);

template <typename... Args>
static constexpr inline void check_logging_overhead() {
  static_assert(
...
struct waived_t {
  ...
  friend Out &&operator <<(Out &&out, waived_t const &what) {
    out << what.what_;
 ...
 */
template <typename T>
constexpr auto log_waive(T &&what) {
...
struct either_t {
  ...
  template <typename Out>
  friend Out &&operator <<(Out &&out, either_t const &what) {
    if (what.condition_) {
      out << what.when_true_;
    } else {
 ...
 */
template <typename WhenTrue, typename WhenFalse>
auto log_either(bool condition, WhenTrue &&when_true, WhenFalse &&when_false) {
...
template <typename T, char Open, char Close = Open>
struct surrounded_t {
  ...
  template <typename Out>
  friend Out &&operator <<(Out &&out, surrounded_t const &what) {
 ...
 */
template <char Open, char Close = Open, typename T>
constexpr auto log_surround(T &&what) {
  logger::check_logging_overhead<T &&>();
  return logger::impl::surrounded_t<T, Open, Close>(std::forward<T>(what));
}

template <typename T>
constexpr auto log_quoted(T &&what) {
  return log_surround<'"'>(std::forward<T>(what));
}

template <typename T>
constexpr auto log_single_quoted(T &&what) {
  return log_surround<'\''>(std::forward<T>(what));
}

template <typename T>
constexpr auto log_parens(T &&what) {
  return log_surround<'(', ')'>(std::forward<T>(what));
}

template <typename T>
constexpr auto log_bracket(T &&what) {
  return log_surround<'[', ']'>(std::forward<T>(what));
}

template <typename T>
constexpr auto log_brace(T &&what) {
  return log_surround<'{', '}'>(std::forward<T>(what));
}
...
template <typename Key, typename Value, char Separator, char Open, char Close = Open>
struct kv_pair_t {
  static constexpr std::string_view invalid_key_characters = "\"='<>{}[]()?/$#@!";
  using key_t = std::decay_t<Key>;

  constexpr explicit kv_pair_t(Key const &key, Value const &value):
    key_(key),
    value_(value)
  {
  ...
  template <typename Out>
  friend Out &&operator <<(Out &&out, kv_pair_t const &what) {
...
constexpr auto log_kv_pair(Key &&key, Value &&value) {
  ...
  return logger::impl::kv_pair_t<Key, Value, Separator, Open, Close>(
    std::forward<Key>(key),
...
struct callable_t {
  ...
  constexpr explicit callable_t(UFn &&fn): fn_(fn) {}
  ...
  template <typename Out>
  friend Out &&operator <<(Out &&out, callable_t const &what) {
    out << what.fn_();
    return std::forward<Out>(out);
  }
 ...
 */
template <typename Fn>
constexpr auto log_call(Fn &&callable) {
