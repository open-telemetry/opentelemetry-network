...
#   endif // ENUM_DEFAULT
  ...
  static constexpr bool is_valid(type value) {
    switch (value) {
  #   define SWITCH_ENUM_TYPES(K,V) case type::K: return true;
   ...
   */
  template <typename T>
  using array_map = std::array<T, count>;
};
...
constexpr inline std::string_view to_string(ENUM_NAME value,
#ifdef ENUM_DEFAULT
  ...
  switch (value) {
#   define SWITCH_ENUM_TYPES(K,V) case ENUM_NAME::K: return #K;
    ENUM_ELEMENTS(SWITCH_ENUM_TYPES)
#   undef SWITCH_ENUM_TYPES
  }
  return fallback;
...
# define IF_ENUM_TYPES(K,V) if(s == #K) { out = ENUM_NAME::K; return true; }
...
constexpr inline ENUM_NAME sanitize_enum(ENUM_NAME value,
