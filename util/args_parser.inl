...
ArgsParser::ArgProxy<T>::ArgProxy(
  ...
  arg_(
    parser, name, description, {name},
    [&]() -> std::conditional_t<std::is_enum_v<T>, std::string, T const &> {
...
template <typename T>
bool ArgsParser::ArgProxy<T>::given() const {
  return arg_.Matched();
}

template <typename T>
typename ArgsParser::ArgProxy<T>::proxy_ref ArgsParser::ArgProxy<T>::operator *() {
  if constexpr (std::is_enum_v<T>) {
    ...
    if (!enum_from_string(arg, value)) {
      throw std::invalid_argument(
        fmt::format("invalid value given for enumeration: '{}'", arg)
      );
    }
    return value;
  } else {
...
template <typename T>
template <typename>
T *ArgsParser::ArgProxy<T>::operator ->() {
  return &arg_.Get();
}

template <typename T>
ArgsParser::ArgProxy<T> ArgsParser::add_arg(
  std::string const &name,
  std::string const &description,
  char const *env_var,
  T default_value
  ...
  if (env_var) {
    if (auto const value = std::getenv(env_var);
      value && !from_string(value, default_value) && *value
    ) {
...
HandlerType &ArgsParser::new_handler(Args &&... args) {
