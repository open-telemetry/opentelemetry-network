...
public:
   ...
   * @param n_watermark_bytes: the number of bytes to wait to arrive before waking up the fd for reading when events are available. special case is 0 = never wake up fd (poll only)
   */
...
inline MmapPerfRingStorage::MmapPerfRingStorage(int cpu, u32 n_bytes, u32 n_watermark_bytes):
  ...
  callback_(nullptr),
  n_watermark_bytes_(n_watermark_bytes)
{
  ...
  if (fd_ == -1) {
    ...
    msg << "perf_event_open on cpu " << cpu << " failed with errno " << errno
  ...
  data_ = (char *)mmap(NULL, mmap_size_, PROT_READ | PROT_WRITE, MAP_SHARED,
                       fd_, 0);
  if (data_ == MAP_FAILED) {
    close(fd_);

    std::stringstream msg;
    msg << "mmap on cpu " << cpu << " failed with errno " << errno
        << ", error: '" << strerror(errno) << "'";
    throw std::runtime_error(msg.str());
  }
  ...
  if (res != 0) {
    ...
    std::stringstream msg;
    msg << "ioctl PERF_EVENT_IOC_ENABLE on cpu " << cpu
        << " failed with return " << res << ", errno " << errno << ", error: '"
        << strerror(errno) << "'";
    throw std::runtime_error(msg.str());
  }
}

inline MmapPerfRingStorage::MmapPerfRingStorage(MmapPerfRingStorage &&other)
    : fd_(other.fd_)
{
...
inline PerfRing::PerfRing(std::shared_ptr<PerfRingStorage> storage)
    : storage_(storage)
{
  int res = pr_init_contig(this, storage->data(), storage->n_data_pages(),
                           storage->page_size());
  if (res != 0) {
    std::stringstream msg;
    msg << "pr_init_contig failed, ret=" << res
        << " storage->data()=" << (u64)storage->data();
    throw std::runtime_error(msg.str());
  }
}
...
inline std::pair<std::string_view, std::string_view> PerfRing::peek() const
{
  ...
  return std::make_pair(
    std::string_view(view.first, view.first_len),
    std::string_view(view.second, view.second_len)
  );
}
