...
#define AUTH_FETCH_ERROR_MESSAGE "double check the following items:\n" \
  "- Agent keys are properly configured:\n" \
  "  - in Kubernetes environments, verify that secret `" AUTH_K8S_SECRET_NAME  \
  "` has been set for namespace `" AUTH_K8S_NAMESPACE "` and contains keys `" \
  AUTH_K8S_KEY_KEY_ID "` and `" AUTH_K8S_KEY_SECRET "`\n" \
  "  - this container's enironment variables `" FLOWMILL_AUTH_KEY_ID_VAR "` and `" \
  FLOWMILL_AUTH_SECRET_VAR "` must contain the agent key\n" \
...
AuthzFetcher::AuthzFetcher(
  ...
  api_url_([&] {
    ...
    auto url = format_url(
      std::move(authz_server),
      fmt::format("api/v1/auth/keys/{}", agent_key.key_id)
    );
  ...
  request_(
    ...
    [this, &url, on_success = std::move(on_success), on_error = std::move(on_error)](
      ...
      if (status != CurlEngineStatus::OK) {
        LOG::error(
          "error [{}] while auto-refreshing authz token from '{}': {}",
          status, url, curlError
        );
        on_error();
  ...
  })
{
  ...
  if (proxy) {
    LOG::trace_in(Utility::authz,
      "setting proxy {}:{} for async fetch",
      proxy->host(), proxy->port()
    );
    request_.http_proxy(
      proxy->host(),
      try_integer_from_string<std::uint16_t>(proxy->port().c_str())
    );
  } else {
...
void AuthzFetcher::ScheduledFetch::schedule(scheduling::Timer::TimerPeriod timeout) {
  ...
  timer_.defer(when).on_error([when](auto const &error) {
    LOG::error("failed to schedule next authz token fetch {} from now: {}", when, error);
  });
}
...
void AuthzFetcher::ScheduledFetch::schedule_backoff() {
  ...
  ++backoff_count_;
  backoff_interval_ = std::min(
    backoff_interval_ * AUTHZ_BACKOFF_GEOMETRIC_RATIO,
    AUTHZ_LONGEST_ERROR_BACKOFF
  );

  timer_.defer(backoff).on_error([&](auto const &error) {
    LOG::error(
      "failed to schedule next authz token fetch after {}-th back-off of {}: {}",
      backoff_count_, backoff, error
    );
  });
}
...
void AuthzFetcher::auto_refresh(uv_loop_t &loop, SuccessCallback on_success) {
  ...
  auto const calculate_delay = [](Expected<AuthzToken, std::runtime_error> const &token) {
    ...
    auto const delay = std::chrono::duration_cast<scheduling::Timer::TimerPeriod>(
      token
        ? token->expiration() - AUTHZ_NOTICE_PERIOD - now
  ...
  scheduled_.emplace(
    ...
    [this, on_success = std::move(on_success), &calculate_delay](std::string_view body) {
      auto const previous_expiration = token_
        ? token_->expiration()
        : AuthzToken::duration::zero();
...
void AuthzFetcher::stop_auto_refresh() {
  if (!scheduled_.has_value()) { return; }
  scheduled_->stop();
}
...
Expected<AuthzToken, std::runtime_error> AuthzFetcher::sync_fetch() {
  ...
  if (proxy_) {
    LOG::trace_in(Utility::authz,
      "setting proxy {}:{} for sync fetch",
      proxy_->host(), proxy_->port()
    );
    fetcher.http_proxy(
      proxy_->host(),
      try_integer_from_string<std::uint16_t>(proxy_->port().c_str())
    );
  } else {
    LOG::trace_in(Utility::authz, "not using a proxy for async fetch");
  }
...
Expected<AuthzFetcher::AgentKey, std::string> AuthzFetcher::read_agent_key() {
  ...
  if (auto key_prefix = key_id.substr(0, AGENT_KEY_PREFIX.size());
