...
inline std::string_view trim_up_to(
  std::string_view &input,
  char what,
...
inline std::size_t count_up_to_last(
  std::string_view input,
  char what,
  bool include
) {
...
inline std::string_view trim_up_to_last(
  std::string_view &input,
  char what,
  SeekBehavior behavior
) {
...
template <typename T>
constexpr T NumberView<T>::value(T fallback) const {
...
template <typename T>
constexpr T NumberView<T>::unpadded_value(T fallback) const {
  ...
  static_assert(
    ...
    "TODO: compute correct buffer size for floating point parsing"
  );
