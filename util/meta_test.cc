...
TEST(flowmill_metadata, field_get) {
  ...
  EXPECT_EQ(0, std::strncmp(
      "abcdef123456789",
      reinterpret_cast<char const *>(pid_info_message_metadata::field_comm::get(&message)),
      16));
}

TEST(flowmill_metadata, message_metadata_for) {
  EXPECT_TRUE((std::is_same_v<
    pid_info_message_metadata,
    ingest_metadata::message_metadata_for<jb_ingest__pid_info>
  >));
  EXPECT_TRUE((std::is_same_v<
    pid_info_message_metadata,
    ingest_metadata::message_metadata_for<jsrv_ingest__pid_info>
  >));
  EXPECT_TRUE((std::is_same_v<
