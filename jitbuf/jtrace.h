...
#define jtrace_register(_jring, msg_name, tracepoint_name)                     \
  ({                                                                           \
    ...
    __jtrace_prototype_##msg_name check_that_prototypes_match =                \
        trace_##tracepoint_name;                                               \
    (void)check_that_prototypes_match; /* suppress unused warning */           \
    /* now register */                                                         \
    switch ((_jring)->backend) {                                               \
    case JTRACE_KRING_BACKEND:                                                 \
      __ret = register_trace_##tracepoint_name(                                \
          __jtrace_kring_probe__##msg_name, (void *)(_jring));                 \
      break;                                                                   \
    case JTRACE_FCP_BACKEND:                                                   \
      __ret = register_trace_##tracepoint_name(__jtrace_fcp_probe__##msg_name, \
                                               (void *)(_jring));              \
      break;                                                                   \
    default:                                                                   \
      __ret = -EINVAL;                                                         \
...
#define jtrace_unregister(_jring, msg_name, tracepoint_name)                   \
  ({                                                                           \
    ...
    switch ((_jring)->backend) {                                               \
    case JTRACE_KRING_BACKEND:                                                 \
      __ret = unregister_trace_##tracepoint_name(                              \
          __jtrace_kring_probe__##msg_name, (void *)(_jring));                 \
      break;                                                                   \
    case JTRACE_FCP_BACKEND:                                                   \
      __ret = unregister_trace_##tracepoint_name(                              \
          __jtrace_fcp_probe__##msg_name, (void *)(_jring));                   \
      break;                                                                   \
    default:                                                                   \
      __ret = -EINVAL;                                                         \
    }                                                                          \
...
#define jtrace_register_err(jring_ptr, msg_name, tracepoint_name)              \
  ({                                                                           \
    ...
    if (__ret != 0)                                                            \
      pr_err("%s: jtrace could not register msg " #msg_name                    \
             " to tracepoint " #tracepoint_name " ret=%d\n",                   \
             __func__, __ret);                                                 \
    __ret;                                                                     \
  })
...
#define jtrace_unregister_notice(jring_ptr, msg_name, tracepoint_name)         \
  ({                                                                           \
    ...
    if (__ret != 0)                                                            \
      pr_notice("%s: jtrace could not unregister msg " #msg_name               \
                " from tracepoint " #tracepoint_name " ret=%d\n",              \
                __func__, __ret);                                              \
    __ret;                                                                     \
  })
