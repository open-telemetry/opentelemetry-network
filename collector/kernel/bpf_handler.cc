...
BPFHandler::BPFHandler(uv_loop_t &loop, std::string full_program,
    ...
    ProcessHandler::CpuMemIoSettings const *cpu_mem_io_settings)
{
  ...
  buf_poller_ = std::make_unique<BufferedPoller>(
      loop_, perf_, buffered_writer, boot_time_adjustment, curl_engine,
      bpf_dump_file_, log_, probe_handler_, bpf_module_, nic_poller,
      cgroup_settings, cpu_mem_io_settings, encoder_);
...
void BPFHandler::load_probes(::flowmill::ingest::Writer &writer)
{
  CgroupProber cgroup_prober(
      probe_handler_, bpf_module_, [this]() { buf_poller_->start(1, 1); },
      [this](std::string error_loc) { check_cb(error_loc); });

  if (cgroup_prober.error_count() > 0) {
    log_.warn("load_probes could not close {} directories",
              cgroup_prober.error_count());
  }
  ...
  NatProber nat_prober(probe_handler_, bpf_module_,
  ...
  int udp_status = probe_handler_.start_probe(bpf_module_, "on_skb_consume_udp",
                                              "skb_consume_udp");
  if (udp_status != 0) {
    ...
    udp_status =
        probe_handler_.start_probe(bpf_module_, "on_skb_free_datagram_locked",
  ...
  if (udp_status != 0) {
    ...
    // v2.6.32-rc6~9^2~3
    udp_status = probe_handler_.start_probe(
        bpf_module_, "on_skb_free_datagram_locked", "skb_free_datagram_locked");
  }
  if (udp_status != 0) {
  ...
  SocketProber socket_prober(
      probe_handler_, bpf_module_, [this]() { buf_poller_->start(1, 1); },
      [this](std::string error_loc) { check_cb(error_loc); }, log_);
  ...
  int rtt_status = probe_handler_.start_probe(
      bpf_module_, "on_tcp_rtt_estimator", "tcp_rtt_estimator");
  if (rtt_status) { // if tcp_rtt_estimator fails
    log_.warn("cannot start tcp_rtt_estimator probe, trying "
              "tcp_update_pacing_rate. error {}",
              rtt_status);

    rtt_status = probe_handler_.start_probe(bpf_module_, "on_tcp_rtt_estimator",
                                            "tcp_update_pacing_rate");
    if (rtt_status) {
      log_.warn(
          "cannot start tcp_update_pacing_rate probe, trying tcp_ack. error {}",
          rtt_status);

      rtt_status = probe_handler_.start_probe(
          bpf_module_, "on_tcp_rtt_estimator", "tcp_ack");
      if (rtt_status) {
  ...
  // SYN timeouts
  int syn_timeout = probe_handler_.start_probe(
      bpf_module_, "on_tcp_retransmit_timer", "tcp_retransmit_timer");
  if (syn_timeout) {
    log_.warn("cannot instrument SYN timeouts. error {}", syn_timeout);
  }

  // SYN-ACK timeouts
  int syn_ack_timeout = probe_handler_.start_probe(
      bpf_module_, "on_tcp_syn_ack_timeout", "tcp_syn_ack_timeout");
  if (syn_ack_timeout) {
  ...
  int tcp_send_active_reset_ret = probe_handler_.start_probe(
      bpf_module_, "on_tcp_send_active_reset", "tcp_send_active_reset");
  if (tcp_send_active_reset_ret) {
    log_.warn("cannot instrument tcp_send_active_reset function: error {}",
              tcp_send_active_reset_ret);
  }
   ...
   */
  int ret;
  ret = probe_handler_.start_probe(bpf_module_, "on_tcp_event_data_recv",
                                   "tcp_event_data_recv");
  if (ret) {
    log_.warn("start tcp_event_data_recv probe failed. error {}", ret);
  }
  ret = probe_handler_.start_probe(bpf_module_, "on_tcp_rcv_established",
                                   "tcp_rcv_established");
  if (ret) {
  ...
  if (probe_handler_.register_tail_call(bpf_module_, "tail_calls",
                                        TAIL_CALL_ON_UDP_SEND_SKB__2,
                                        "on_udp_send_skb__2") != 0) {
    log_.error("failed to register tail call for on_udp_send_skb__2");
  }
  if (probe_handler_.register_tail_call(bpf_module_, "tail_calls",
                                        TAIL_CALL_ON_UDP_V6_SEND_SKB__2,
                                        "on_udp_v6_send_skb__2") != 0) {
    log_.error("failed to register tail call for on_udp_v6_send_skb__2");
  }
  if (probe_handler_.register_tail_call(bpf_module_, "tail_calls",
                                        TAIL_CALL_ON_IP_SEND_SKB__2,
                                        "on_ip_send_skb__2") != 0) {
    log_.error("failed to register tail call for on_ip_send_skb__2");
  }
  if (probe_handler_.register_tail_call(bpf_module_, "tail_calls",
                                        TAIL_CALL_ON_IP6_SEND_SKB__2,
                                        "on_ip6_send_skb__2") != 0) {
    log_.error("failed to register tail call for on_ip6_send_skb__2");
  }
  if (probe_handler_.register_tail_call(bpf_module_, "tail_calls",
                                        TAIL_CALL_HANDLE_RECEIVE_UDP_SKB,
                                        "handle_receive_udp_skb") != 0) {
    log_.error("failed to register tail call for handle_receive_udp_skb");
  }
  if (probe_handler_.register_tail_call(bpf_module_, "tail_calls",
                                        TAIL_CALL_HANDLE_RECEIVE_UDP_SKB__2,
                                        "handle_receive_udp_skb__2") != 0) {
    log_.error("failed to register tail call for handle_receive_udp_skb__2");
  }
  if (probe_handler_.register_tail_call(bpf_module_, "tail_calls",
                                        TAIL_CALL_CONTINUE_TCP_SENDMSG,
                                        "continue_tcp_sendmsg") != 0) {
    log_.error("failed to register tail call for continue_tcp_sendmsg");
  }
  if (probe_handler_.register_tail_call(bpf_module_, "tail_calls",
                                        TAIL_CALL_CONTINUE_TCP_RECVMSG,
                                        "continue_tcp_recvmsg") != 0) {
    log_.error("failed to register tail call for continue_tcp_recvmsg");
  }

  // udp v4 send statistics and dns requests
  int udp_send_skb_status = probe_handler_.start_probe(
      bpf_module_, "on_udp_send_skb", "udp_send_skb");
  if (udp_send_skb_status != 0) {
    log_.warn("udp_send_skb probe failed, using ip_send_skb hook. error {}",
              udp_send_skb_status);
    int ip_send_skb_status = probe_handler_.start_probe(
        bpf_module_, "on_ip_send_skb", "ip_send_skb");
  ...
  int udp_v6_send_skb_status = probe_handler_.start_probe(
      bpf_module_, "on_udp_v6_send_skb", "udp_v6_send_skb");
  if (udp_v6_send_skb_status != 0) {
    log_.warn("udp_v6_send_skb probe failed, using ip6_send_skb hook. error {}",
              udp_v6_send_skb_status);
    int ip6_send_skb_status = probe_handler_.start_probe(
        bpf_module_, "on_ip6_send_skb", "ip6_send_skb");
  ...
  if (enable_http_metrics_ /*|| any other tcp-processor flags eventually */) {
    ...
    probe_handler_.start_probe(bpf_module_, "handle_kprobe__tcp_init_sock",
                               "tcp_init_sock", "_tcpproc");
    probe_handler_.start_probe(bpf_module_, "handle_kprobe__security_sk_free",
                               "security_sk_free", "_tcpproc");
    probe_handler_.start_kretprobe(bpf_module_,
                                   "handle_kretprobe__inet_csk_accept",
                                   "inet_csk_accept", "_tcpproc");
    probe_handler_.start_probe(bpf_module_, "handle_kprobe__inet_csk_accept",
                               "inet_csk_accept", "_tcpproc");
    probe_handler_.start_kretprobe(bpf_module_, "handle_kretprobe__tcp_sendmsg",
                                   "tcp_sendmsg", "_tcpproc");
    probe_handler_.start_probe(bpf_module_, "handle_kprobe__tcp_sendmsg",
                               "tcp_sendmsg", "_tcpproc");
    probe_handler_.start_kretprobe(bpf_module_, "handle_kretprobe__tcp_recvmsg",
                                   "tcp_recvmsg", "_tcpproc");
    probe_handler_.start_probe(bpf_module_, "handle_kprobe__tcp_recvmsg",
                               "tcp_recvmsg", "_tcpproc");
...
void BPFHandler::check_cb(std::string error_loc)
{
  ...
  if (lost_count > last_lost_count_) {
    ...
    log_.warn("after {}, cumulative lost count non-zero: {} new, {} total",
              error_loc, diff_lost_count, lost_count);
  }
}
