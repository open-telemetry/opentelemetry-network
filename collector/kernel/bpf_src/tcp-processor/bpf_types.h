...
#define GET_PID_TGID PID_TGID _pid_tgid = bpf_get_current_pid_tgid();  \
  TGID _tgid = TGID_FROM_PID_TGID(_pid_tgid); \
  PID _pid = PID_FROM_PID_TGID(_pid_tgid);  \
  if(_tgid==0 || _pid==0) { bpf_log(ctx, BPF_LOG_INVALID_PID_TGID, _pid_tgid, 0, 0); } \
...
#define END_SAVE_ARGS(FUNC)                                                     \
    ...
    if(__ret==-E2BIG || __ret==-ENOMEM || __ret==-EINVAL) {                             \
      ...
      __DEBUG_OTHER_MAP_ERRORS_PRINTK || bpf_trace_printk(#FUNC ": unknown return code from map insert: ret=%d (tgid=%u)\n", __ret, _tgid);  \
    } \
  }
...
inline static struct in6_addr make_ipv6_address(__be32 addr) {
  struct in6_addr addr6 = {
        .in6_u.u6_addr32 = {0, 0, 0xffff0000, addr}
  };
  return addr6;
}
