...
#define END_SAVE_ARGS(FUNC)                                                                                                    \
  ...
  if (__ret != 0 && __DEBUG_OTHER_MAP_ERRORS_PRINTK) {                                                                         \
    ...
    if (existing) {                                                                                                            \
      bpf_trace_printk(#FUNC ": duplicate arg insert. pid_tgid=%lld", (unsigned long long)(bpf_get_current_pid_tgid()));        \
    } else {                                                                                                                   \
      bpf_trace_printk(#FUNC ": args table is full, dropped insert. pid_tgid=%lld", (unsigned long long)(bpf_get_current_pid_tgid()));                                                          \
    }                                                                                                                          \
  }                                                                                                                            \
  }
