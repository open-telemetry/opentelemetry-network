...
#define TABLE_SIZE__SEEN_INODES 70000               // XXX: Is this even necessary? could this tracking be done in userland with non-limited tables?
...
#define TABLE_SIZE__SEEN_CONNTRACKS (TABLE_SIZE__TCP_OPEN_SOCKETS + TABLE_SIZE__UDP_OPEN_SOCKETS)   // Worst case scenario, we have a conntrack for every open socket
...
#define WATERMARK_STACK_TRACES (TABLE_SIZE__STACK_TRACES - 256) // When to clear the table (unfortunately non-atomic, but that's a lot of stack traces...)
