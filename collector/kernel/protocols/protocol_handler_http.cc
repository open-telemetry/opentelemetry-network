...
{
  LOG::debug_in(AgentLogKind::PROTOCOL,"ProtocolHandler_HTTP::$ctor(sk={:x})", 
    control_key().sk);
}

ProtocolHandler_HTTP::~ProtocolHandler_HTTP()
{
  LOG::debug_in(AgentLogKind::PROTOCOL,"ProtocolHandler_HTTP::$dtor(sk={:x})", 
    control_key().sk);
}
...
{
  LOG::debug_in(AgentLogKind::PROTOCOL,"ProtocolHandler_HTTP::handle_server_data(sk={:x}): tstamp={}, offset={}, is_recv={}, data_len={}, data:\n{}", 
  ...
  if(offset > server_stream_position_) {
    LOG::debug_in(AgentLogKind::PROTOCOL,"ProtocolHandler_HTTP::handle_server_data: server stream skipped {} bytes from {} to {}",
      offset - server_stream_position_,
      server_stream_position_,
      offset);
  }
  else if(offset > server_stream_position_) {
    LOG::debug_in(AgentLogKind::PROTOCOL,"ProtocolHandler_HTTP::handle_server_data: server stream went backward {} bytes from {} to {}",
  ...
  while(server_state_ != SERVER_STATE::END) {
    ...
    switch(server_state_) {
      ...
      case SERVER_STATE::VERSION_AND_CODE:
      {
        ...
        LOG::debug_in(AgentLogKind::HTTP,
          "handle_http_response: tstamp={}, sk={:x}, pid={}, code={}, "
          "latency_ns={}, client_server={}",
          response_timestamp_, control_key().sk, pid(), http_code_, latency,
          client_server_type_to_string(client_server));

        data_handler()->writer().http_response_tstamp(response_timestamp_, control_key().sk, pid(), http_code_, latency, (u8)client_server);
...
{
  LOG::debug_in(AgentLogKind::PROTOCOL,"ProtocolHandler_HTTP::handle_client_data(sk={:x}): tstamp={}, offset={}, is_recv={}, data_len={}, data:\n{}", 
    tstamp, control_key().sk, offset, stream_type_to_string(stream_type), data_len, std::string_view((const char *)data, data_len));
  ...
  if(offset > client_stream_position_) {
    LOG::debug_in(AgentLogKind::PROTOCOL,"ProtocolHandler_HTTP::handle_client_data: client stream skipped {} bytes from {} to {}",
      offset - client_stream_position_,
      client_stream_position_,
      offset);
  }
  else if(offset > client_stream_position_) {
    LOG::debug_in(AgentLogKind::PROTOCOL,"ProtocolHandler_HTTP::handle_client_data: client stream went backward {} bytes from {} to {}",
